"""
Original code: Sergi Caelles (scaelles@vision.ee.ethz.ch)

Modified code: Miriam Bellver (miriam.bellver@bsc.es)
...
class Dataset:
    def __init__(self, train_list, test_list, val_list, database_root, number_of_slices, store_memory=True):
        ...
        self.liver_result_train = []
        self.liver_result_train_path = []
        # self.betas_train = []
        # self.gammas_train = []
            ...
            if (len(line.split()) > 3):
                if store_memory:
                    ...
                    for i in range(number_of_slices):
                        aux_images_train.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4])))['section'],
                                                      dtype=np.float32))
                    self.images_train.append(np.array(aux_images_train))
                    
                    for i in range(number_of_slices):
                        aux_labels_train.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4 +1])))['section'],
                                                      dtype=np.float32))
                    self.labels_train.append(np.array(aux_labels_train))
                    
                    for i in range(number_of_slices):
                        aux_labels_liver_train.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4 +2])))['section'],
                                                      dtype=np.float32))
                    self.labels_liver_train.append(np.array(aux_labels_liver_train))
                    
                    for i in range(number_of_slices):
                        aux_liver_result_train.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4 +3])))['section'],
                                                      dtype=np.float32))
                    self.liver_result_train.append(np.array(aux_liver_result_train))
                    
                                  
                    if (idx+1) % 1000 == 0:
                        print('Loaded ' + str(idx) + ' train images')
                # self.betas_train.append(np.array(float(line.split()[-2]), dtype=np.float32))
                # self.gammas_train.append(np.array(float(line.split()[-1]), dtype=np.float32))
                
                aux_images_train_path = []
                aux_labels_train_path = []
                aux_labels_liver_train_path = []
                aux_liver_result_train_path = []
                
                for i in range(number_of_slices):
                    aux_images_train_path.append(os.path.join(database_root, str(line.split()[i*4])))
                self.images_train_path.append(np.array(aux_images_train_path))
                    
                for i in range(number_of_slices):
                    aux_labels_train_path.append(os.path.join(database_root, str(line.split()[i*4 + 1])))
                self.labels_train_path.append(np.array(aux_labels_train_path))
                
                for i in range(number_of_slices):
                    aux_labels_liver_train_path.append(os.path.join(database_root, str(line.split()[i*4 + 2])))
                self.labels_liver_train_path.append(np.array(aux_labels_liver_train_path))
                
                for i in range(number_of_slices):
                    aux_liver_result_train_path.append(os.path.join(database_root,str(line.split()[i*4 + 3])))
                self.liver_result_train_path.append(np.array(aux_liver_result_train_path))
        ...
        self.liver_result_train_path = np.array(self.liver_result_train_path)

        # Load testing images (path) and labels
        self.images_test = []
        self.images_test_path = []
        self.liver_result_test = []
        self.liver_result_test_path = []
        ...
        for idx, line in enumerate(test_paths):
            if (len(line.split()) > 1):
                if store_memory:
                    aux_images_test = []
                    for i in range(number_of_slices):
                        aux_images_test.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*3])))['section'],
                                                      dtype=np.float32))
                    self.images_test.append(np.array(aux_images_test))
                    aux_liver_result_test = []
                    for i in range(number_of_slices):
                        aux_liver_result_test.append(np.array(scipy.io.loadmat(os.path.join(database_root,str(line.split()[i*3 + 3])))['section'],
                                                      dtype=np.float32))
                    self.liver_result_test.append(np.array(aux_liver_result_test))
                ...
                aux_liver_result_test_path = []
                for i in range(number_of_slices):
                    aux_liver_result_test_path.append(os.path.join(database_root,str(line.split()[i*3 + 3])))
                self.liver_result_test_path.append(np.array(aux_liver_result_test_path))
        ...
        self.liver_result_val = []
        self.liver_result_val_path = []
        # self.betas_val = []
        # self.gammas_val = []
        for idx, line in enumerate(val_paths):
            if (len(line.split()) > 3):
                if store_memory:
                    ...
                    aux_liver_result_val = []
                    for i in range(number_of_slices):
                        aux_images_val.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4])))['section'],
                                                      dtype=np.float32))
                    self.images_val.append(np.array(aux_images_val))
                    
                    for i in range(number_of_slices):
                        aux_labels_val.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4 +1])))['section'],
                                                      dtype=np.float32))
                    self.labels_val.append(np.array(aux_labels_val))
                    
                    for i in range(number_of_slices):
                        aux_labels_liver_val.append(np.array(scipy.io.loadmat(os.path.join(database_root, str(line.split()[i*4 +2])))['section'],
                                                      dtype=np.float32))
                    self.labels_liver_val.append(np.array(aux_labels_liver_val))
                    
                    for i in range(number_of_slices):
                        aux_liver_result_val.append(np.array(scipy.io.loadmat(os.path.join(database_root,str(line.split()[i*4 +3])))['section'],
                                                      dtype=np.float32))
                    self.liver_result_val.append(np.array(aux_liver_result_val))
                    
                                  
                    if (idx+1) % 1000 == 0:
                        print('Loaded ' + str(idx) + ' train images')
                # self.betas_val.append(np.array(float(line.split()[-2]), dtype=np.float32))
                # self.gammas_val.append(np.array(float(line.split()[-1]), dtype=np.float32))
                
                aux_images_val_path = []
                aux_labels_val_path = []
                aux_labels_liver_val_path = []
                aux_liver_result_val_path = []
                
                for i in range(number_of_slices):
                    aux_images_val_path.append(os.path.join(database_root, str(line.split()[i*4])))
                self.images_val_path.append(np.array(aux_images_val_path))
                    
                for i in range(number_of_slices):
                    aux_labels_val_path.append(os.path.join(database_root, str(line.split()[i*4 + 1])))
                self.labels_val_path.append(np.array(aux_labels_val_path))
                
                for i in range(number_of_slices):
                    aux_labels_liver_val_path.append(os.path.join(database_root, str(line.split()[i*4 + 2])))
                self.labels_liver_val_path.append(np.array(aux_labels_liver_val_path))
                
                for i in range(number_of_slices):
                    aux_liver_result_val_path.append(os.path.join(database_root,str(line.split()[i*4 + 3])))
                self.liver_result_val_path.append(np.array(aux_liver_result_val_path))
        ...
        self.liver_result_val_path = np.array(self.liver_result_val_path)
        ...
        if phase == 'train':
            if self.train_ptr + batch_size < self.train_size:
                ...
                if self.store_memory:
                    ...
                    labels_liver = [self.labels_liver_train[l] for l in idx]
                    results_liver = [self.liver_result_train[l] for l in idx]
                else:
                    ...
                    results_liver = [self.liver_result_train_path[l] for l in idx]
                ...
                if self.store_memory:
                    ...
                    results_liver_1 = [self.liver_result_train[l] for l in old_idx]
                    ...
                    results_liver_2 = [self.liver_result_train[l] for l in idx]
                else:
                    ...
                    results_liver_1 = [self.liver_result_train_path[l] for l in old_idx]
                    ...
                    results_liver_2 = [self.liver_result_train_path[l] for l in idx]
                images = images_1+images_2
                labels = labels_1+labels_2
                labels_liver = labels_liver_1 + labels_liver_2
                # betas = betas_1 + betas_2
                # gammas = gammas_1 + gammas_2
                results_liver = results_liver_1 + results_liver_2
                self.train_ptr = new_ptr
            return images, labels, labels_liver, results_liver
        if phase == 'val':
            if self.val_ptr + batch_size < self.val_size:
                ...
                if self.store_memory:
                    ...
                    results_liver = [self.liver_result_val[l] for l in idx]
                else:
                    ...
                    results_liver = [self.liver_result_val_path[l] for l in idx]
                ...
                if self.store_memory:
                    ...
                    results_liver_1 = [self.liver_result_val[l] for l in old_idx]
                    ...
                    results_liver_2 = [self.liver_result_val[l] for l in idx]
                else:
                    ...
                    results_liver_1 = [self.liver_result_val_path[l] for l in old_idx]
                    ...
                    results_liver_2 = [self.liver_result_val_path[l] for l in idx]
                images = images_1+images_2
                labels = labels_1+labels_2
                labels_liver = labels_liver_1 + labels_liver_2
                results_liver = results_liver_1 + results_liver_2
            ...
            if self.test_ptr + batch_size < self.test_size:
                if self.store_memory:
                    ...
                    liver_results = self.liver_result_test[self.test_ptr:self.test_ptr + batch_size]
                paths = self.images_test_path[self.test_ptr:self.test_ptr + batch_size]
                liver_results_paths = self.liver_result_test_path[self.test_ptr:self.test_ptr + batch_size]
                self.test_ptr += batch_size
            else:
                new_ptr = (self.test_ptr + batch_size) % self.test_size
                if self.store_memory:
                    images = self.images_test[self.test_ptr:] + self.images_test[:new_ptr]
                    liver_results = self.liver_result_test[self.test_ptr:] + self.liver_results_test[:new_ptr]
                paths = self.images_test_path[self.test_ptr:] + self.images_test_path[:new_ptr]
                liver_results_paths = self.liver_result_test_path[self.test_ptr:] + self.images_test_path[:new_ptr]
                self.test_ptr = new_ptr
            return images, liver_results, paths, liver_results_paths
        else:
